...
function throwOnTooLargeNeedle(
	...
	if (scaledRows > haystack.rows || scaledCols > haystack.cols) {
		throw new Error(
			"Search input is too large, try using a smaller template image."
		);
	}
}
...
export default class TemplateMatchingFinder implements ImageFinderInterface {
	...
	public async findMatches<PROVIDER_DATA_TYPE>(
		matchRequest: MatchRequest<Image, PROVIDER_DATA_TYPE>
		...
		throwOnTooLargeNeedle(
			haystack,
			needle,
			this.scaleSteps[this.scaleSteps.length - 1]
		);
		...
		if (
			(matchRequest.providerData as TemplateMatcherProviderData)
		...
		const matches = await Promise.all(matchResults).then((results) => {
			results.forEach((matchResult) => {
				matchResult.location.left /=
					matchRequest.haystack.pixelDensity.scaleX;
				matchResult.location.width /=
					matchRequest.haystack.pixelDensity.scaleX;
				matchResult.location.top /=
					matchRequest.haystack.pixelDensity.scaleY;
		...
		const potentialMatches = matches.filter(
			(match) => match.confidence >= matchRequest.confidence
		);
		if (potentialMatches.length === 0) {
			...
			if (bestMatch) {
				throw new Error(
					`No match with required confidence ${matchRequest.confidence}. Best match: ${bestMatch.confidence}`
				);
			} else {
				throw new Error(
					`Unable to locate ${matchRequest.needle.id}, no match!`
				);
			}
		}
	...
	public async findMatch<PROVIDER_DATA_TYPE>(
		matchRequest: MatchRequest<Image, PROVIDER_DATA_TYPE>
	): Promise<MatchResult<Region>> {
	...
	private async searchMultipleScales(needle: cv.Mat, haystack: cv.Mat) {
		...
		for (const currentScale of this.scaleSteps) {
			...
			if (
				scaledHaystack.cols < needle.cols ||
			...
			if (
				haystack.cols < scaledNeedle.cols ||
			...
			const matchHaystackResult = await matchImages(
				scaledHaystack,
				needle
			);
			results.push(
				new MatchResult(
					...
					scaleLocation(matchHaystackResult.location, currentScale)
				)
			);
		}
		return results;
